...
export function CodeConceptCard({
  ...
  return (
    <div className="manual-tutorial-step-card w-full text-center">
      ...
      <div className="manual-guidance-monaco relative mx-auto mt-5 h-[150px] w-full max-w-md select-none overflow-hidden rounded-xl border border-white/10 bg-[#0F151B] shadow-none">
          ...
          <div className="absolute right-2 top-2 z-10 flex max-w-[calc(100%-1rem)] flex-col items-end gap-1">
            {actionOptions.map((option, index) => (
              <button
                ...
                disabled={disabled}
                className="inline-flex h-7 items-center justify-center rounded-full border border-cyan-400/20 bg-cyan-400/10 px-2 text-[10px] font-semibold text-cyan-200/70 transition-colors duration-200 ease-out hover:bg-cyan-400/20 hover:text-cyan-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-200 disabled:cursor-not-allowed disabled:opacity-40 motion-reduce:transition-none"
              >
                {option.label}
              </button>
            ))}
