...
import type { QuizOptionState, QuizOptionViewModel } from "@/features/quizzes/components/types";
...
export function SingleChoiceQuestion({
  ...
  const options: QuizOptionViewModel[] = (question.options ?? []).map((option) => ({
    id: option.optionId,
    content: <RenderableContent content={option.content} className="[&>*]:mb-0 text-sm" />,
    feedback: (
      <RenderableContent
