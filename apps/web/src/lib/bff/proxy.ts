...
export function buildBackendHeaders(
  ...
  if (context.studySlug && token) {
    headers.set("x-aalie-study-slug", context.studySlug);
  }
  ...
  if (path === "/analyze/open") {
    ...
    if (typeof record.preferred_method === "string") {
      headers.set(
        "x-aalie-analysis-method",
        record.preferred_method.slice(0, 64),
      );
    }
  }
  return headers;
...
export async function proxyApiRequest(
  ...
  try {
    ...
    const token = context.authenticated
      ? await mintInternalJwt(request.headers)
      : null;
...
export async function proxyJsonRequest(
  ...
  return proxyApiRequest(request, options);
}

export function clearStudyCookie(response: NextResponse): NextResponse {
  response.cookies.set(STUDY_COOKIE_NAME, "", {
    path: "/",
    maxAge: 0,
    httpOnly: true,
    sameSite: "lax",
