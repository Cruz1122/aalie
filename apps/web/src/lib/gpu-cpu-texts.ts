  ...
  en: {
    ...
    explanation: {
      loopsDetected: (n: number) =>
        `• ${n} loop${n !== 1 ? "s" : ""} detected in total`,
    ...
    recommendations: {
      ...
      mixed: [
        ...
        "Recommendation: the mixed structure of this algorithm suggests that a modern multicore CPU is the best initial option. If you need to scale later, identify the most regular sections (simple loops) and consider parallelizing them, keeping complex logic on CPU.",
        ...
        "Recommendation: the mixed nature of this algorithm makes a multicore CPU the most versatile option. For additional improvements, you could identify and parallelize only the most regular sections, keeping the rest of the processing on CPU where control flow is more efficient.",
      ],
    },
  },
  es: {
    ...
    explanation: {
      loopsDetected: (n: number) =>
        `• Se detectaron ${n} bucle${n !== 1 ? "s" : ""} en total`,
      depthNested: (d: number) =>
        d === 2
