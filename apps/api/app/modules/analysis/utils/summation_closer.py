...
class SummationCloser:
    ...
    def __init__(
        ...
        self._exclude_from_iteration_substitution = set(
            exclude_from_iteration_substitution or set()
        )
    ...
    def _builtin_iteration_names(self) -> List[str]:
        return [
            x
            for x in ("i", "j", "k")
            if x not in self._exclude_from_iteration_substitution
        ]
    ...
    def _safe_substitute_iteration_var(
        self, expr: Expr, var_symbol: Symbol, var_name: str, variable: str
    ) -> Expr:
        """
        Sustituye variable de iteración por 0 o por variable principal.
        No sustituye por 0 si el resultado sería negativo (evita bugs como n*(j-1)→-n).
        ...
        main_sym = next(
            (
                ...
                if getattr(s, "name", None) == variable
            ),
            None,
        )
        if main_sym is None:
            ...
            try:
                ...
                if has_sum:
                    ...
                    result_expr = result_expr.replace(
                        lambda x: isinstance(x, Sum), lambda x: x.doit()
                    )
                    result_expr = simplify(result_expr)
                ...
                for var_name in iteration_vars:
                    var_symbol = next(
                        (
                            s
                            for s in result_expr.free_symbols
                            if getattr(s, "name", None) == var_name
                        ),
                        None,
                    )
                    ...
                    if result_expr.has(var_symbol):
                        ...
                        try:
                            ...
                            if result_expr.has(var_symbol):
                                ...
                                if result_expr.has(var_symbol):
                                    ...
                                    if isinstance(result_expr, SymSum):
                                        ...
                                        result_expr = expand(result_expr)
                                        result_expr = simplify(result_expr)
                                # Si todavía queda después de todo, sustituir (0 o variable principal)
                                if result_expr.has(var_symbol):
                                    from sympy import Integer as SymInteger
                            ...
                            print(
                                f"[SummationCloser] Error eliminando variable de iteración {var_name}: {e}"
                            )
                ...
                if iter_latex_markers and any(
                    m in closed_latex for m in iter_latex_markers
                ):
                    print(
        ...
        while "\\sum" in expr:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"[SummationCloser] Error usando summation(), intentando con Sum().doit(): {e1}"
                    )
                ...
                if evaluated_str.startswith(")"):
                    ...
                    print(
                        f"[SummationCloser] WARNING: evaluated_str empieza con ')': '{evaluated_str}'"
                    )
                ...
                replaced_part = (
                    expr[innermost_match.start() : body_start + len(body_str)]
    ...
    def _parse_algebraic_to_sympy(self, expr: str, variable: str) -> "Expr":
        ...
        prev_expr = ""
        while (
            expr_normalized.startswith("(")
            and expr_normalized.endswith(")")
            and iteration < max_iterations
        ):
            iteration += 1
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[SummationCloser] Error parseando después de simplificar paréntesis {expr_normalized}: {e2}"
                )
    ...
    def _sympy_to_latex(self, expr: "Expr") -> str:
        ...
        Asegura paréntesis en productos a·b cuando b es suma/resta (Add/Sub).
        ...
        try:
            ...
            # Usar printer que envuelve Add/Sub en paréntesis cuando son factores de Mul
            latex_str = self._latex_with_mul_parens(cleaned_expr)
            # Normalizar formato
    ...
    def _latex_with_mul_parens(self, expr: "Expr") -> str:
        """
        Convierte a LaTeX asegurando paréntesis en Mul cuando un factor es Add.
                        ...
                        if len(limits) >= 3:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    print(
                                        f"[SummationCloser] Error evaluando sumatoria: {e1}, {e2}"
                                    )
                                    return expr
                        else:
        ...
        for subexpr in preorder_traversal(expr):
            if isinstance(subexpr, Sum):
                ...
                for existing_sum in sums_found:
                    if (
                        isinstance(existing_sum, Sum)
                        and isinstance(subexpr, Sum)
                        and len(existing_sum.args) == len(subexpr.args)
                        and existing_sum.args[0] == subexpr.args[0]
                        and existing_sum.args[1] == subexpr.args[1]
                    ):
                        is_duplicate = True
                        break
                if not is_duplicate:
    ...
    def _analyze_single_sum(
        self, sum_expr: Sum, variable: str = "n"
    ) -> tuple[List[str], Optional[Expr]]:
        """
        ...
        try:
            ...
            if len(sum_expr.args) >= 2:
                ...
                try:
                    if (
                        len(limits) >= 3
                        and limits[0] is not None
                        and limits[1] is not None
                        and limits[2] is not None
                    ):
                        sum_var = limits[0]
                        start = limits[1]
                        end = limits[2]

                    # Convertir a LaTeX para mostrar (ParensLatexPrinter evita ambigüedad n·-i+n)
                    var_latex = self._sympy_to_latex(sum_var)
                    start_latex = self._sympy_to_latex(start)
                    end_latex = self._sympy_to_latex(end)
                    body_latex = self._sympy_to_latex(body)
                    ...
                    if (
                        not is_arithmetic_sum
                        and isinstance(body, SymSymbol)
                        and isinstance(sum_var, SymSymbol)
                    ):
                        is_arithmetic_sum = body.name == sum_var.name

                    body_depends_on_var = (
                        body.has(sum_var) if isinstance(sum_var, SymSymbol) else False
                    )
                    ...
                    if body == Int(1) or body == 1:
                        ...
                        formula_latex = self._sympy_to_latex(formula_expr)
                    ...
                    # DEBE ir ANTES del caso de factor constante
                        ...
                        try:
                                ...
                                try:
                                    ...
                                    if isinstance(start, Integer):
                                        ...
                                        if result_manual_latex != result_latex:
                                            ...
                                            if (
                                                not result_manual.has(sum_var)
                                                if isinstance(sum_var, SymSymbol)
                                                else True
                                            ):
                            ...
                            print(
                                f"[SummationCloser] Error calculando suma aritmética con summation(): {e}"
                            )
                            import traceback
                    ...
                    # Si el cuerpo NO depende de la variable de suma, es constante (factor constante)
                        ...
                        result_simplified = simplify(result_expr)
                        result_latex = self._sympy_to_latex(result_simplified)
                        body_latex_display = self._sympy_to_latex(body)
                    ...
                    # Si el cuerpo depende de la variable de forma lineal (como -i + n + 1)
                        ...
                        try:
                            ...
                            evaluated_latex = self._sympy_to_latex(evaluated_simplified)
                    ...
                    try:
                        sum_latex = self._sympy_to_latex(sum_expr)
                        steps.append(sum_latex)
                    except Exception:
        ...
        if len(sums) >= 2:
            ...
            if isinstance(outer_limits, tuple) and len(outer_limits) >= 3:
                ...
                if isinstance(inner_limits, tuple) and len(inner_limits) >= 3:
                    ...
                    # Convertir a LaTeX
                    outer_var_latex = self._sympy_to_latex(outer_var)
                    outer_start_latex = self._sympy_to_latex(outer_start)
                    outer_end_latex = self._sympy_to_latex(outer_end)
                    inner_var_latex = self._sympy_to_latex(inner_var)
                    inner_start_latex = self._sympy_to_latex(inner_start)
                    inner_end_latex = self._sympy_to_latex(inner_end)
                    inner_body_latex = self._sympy_to_latex(inner_body)
                        ...
                        try:
                            ...
                            if not inner_result_expr.has(outer_var):
                                ...
                                try:
                                    ...
                                    outer_result = summation(
                                        inner_result_simplified,
                                        (outer_var, outer_start, outer_end),
                                    )
                                    ...
                                    if (
                                        ...
                                        or "j" in outer_result_latex
                                        or "k" in outer_result_latex
                                    ):
                                        # Intentar evaluar de nuevo si el LaTeX todavía tiene variables
                                        print(
                                            f"[SummationCloser] Advertencia: Resultado LaTeX todavía contiene variables de iteración: {outer_result_latex}"
                                        )

                                    steps.append(outer_result_latex)
                                except Exception as eval_error:
                                    print(
                                        f"[SummationCloser] Error evaluando sumatoria externa: {eval_error}"
                                    )
                        except Exception as e:
                            print(
                                f"[SummationCloser] Error en análisis de sumatorias anidadas: {e}"
                            )
        else:
    ...
    def _analyze_nested_sum_with_structure(
        self, outer_sum: Sum, inner_sum: Sum, variable: str = "n"
    ) -> List[str]:
        """
    ...
    def _analyze_multiple_limits_sum(
        ...
        if len(all_limits) >= 2:
            ...
            try:
                        ...
                        limits_info.append(
                            {
                                ...
                                "start": start,
                                "end": end,
                                "var_latex": self._sympy_to_latex(var),
                                "start_latex": self._sympy_to_latex(start),
                                "end_latex": self._sympy_to_latex(end),
                            }
                        )
                ...
                for limit_info in reversed(
                    limits_info
                ):  # Empezar con interno ([-1]), terminar con externo ([0])
                ...
                if body == Int(1) or body == 1:
                    ...
                    inner_result_unsimplified_latex = self._sympy_to_latex(
                        inner_result_unsimplified
                    )
                ...
                try:
                    ...
                    if any(
                        ...
                        print(
                            f"[SummationCloser] Advertencia: Resultado LaTeX todavía contiene variables de iteración: {evaluated_latex}"
                        )
