...
def normalize_recursive_formula(formula: Optional[str]) -> Optional[str]:
    ...
    for pattern, replacement in (
        (
            ...
            r"\text{Cada nivel tiene costo: } n \quad \text{Total: }",
        ),
        (
            re.compile(
            ...
            r"\text{Each level has cost: } n \quad \text{Total: }",
        ),
    ):
        match = pattern.search(formula)
        if match:
            # Avoid re.sub replacement escape pitfalls with LaTeX commands (e.g. \quad).
