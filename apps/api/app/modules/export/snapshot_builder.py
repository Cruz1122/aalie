        ...
        nodes.append(
            {
                ...
                "data": {
                    ...
                    "microseconds": data.get("microseconds")
...
def _normalize_trace_graph_from_call_tree(trace: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for call in calls:
        ...
        params_str = ", ".join(f"{key}={json.dumps(value, ensure_ascii=False)}" for key, value in params.items())
...
def _resolve_report_trace_graph(trace: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if not trace or not trace.get("ok"):
        return None
    return _normalize_trace_graph_from_structured(trace) or _normalize_trace_graph_from_call_tree(trace)
                ...
                warnings.append(
                    {
                        ...
                        "message": ((trace or {}).get("errors") or [{}])[0].get("message")
...
def build_snapshot(snapshot_input: Dict[str, Any], options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    parse_payload = snapshot_input.get("parse") if isinstance(snapshot_input.get("parse"), dict) else None
    analyze_payload = snapshot_input.get("analyze") if isinstance(snapshot_input.get("analyze"), dict) else None
    ...
    method_details = [
        {"method": "characteristic_equation", "detail": totals.get("characteristic_equation")}
    ...
    trace_by_case = snapshot_input.get("traceByCase") if isinstance(snapshot_input.get("traceByCase"), dict) else {}
    trace_summary_data = []
    for case_name, trace in trace_by_case.items():
        if not isinstance(trace, dict):
            continue
        ...
        diagnostics = trace_data.get("diagnostics") if isinstance(trace_data.get("diagnostics"), dict) else {}
    ...
    snapshot_without_hash: Dict[str, Any] = {
        ...
        "input": {
            ...
            "analysisSummary": {
                ...
                "availableCases": [
                    case_name
                    for case_name, value in normalized_cases.items()
                    if value
                ],
            },
            "traceSummary": create_section("available", trace_summary_data)
            if trace_summary_data
            else create_section("not_requested"),
        },
        "internal": {
            "ast": create_section("available", parse_ast)
            if parse_ast
            else create_section("missing_data"),
            "classification": create_section(
                "available",
                {
            ...
            "intermediateMath": create_section(
                "available",
                {
                    "proof": (totals or {}).get("proof"),
                    "characteristicEquation": (totals or {}).get("characteristic_equation"),
                    "characteristicEquationStepByStep": ((totals or {}).get("characteristic_equation") or {}).get("step_by_step"),
                    "iteration": (totals or {}).get("iteration"),
                    "iterationStepByStep": ((totals or {}).get("iteration") or {}).get("step_by_step"),
                    "master": (totals or {}).get("master"),
                    "masterStepByStep": ((totals or {}).get("master") or {}).get("step_by_step"),
                    "recursionTree": (totals or {}).get("recursion_tree"),
                    "recursionTreeStepByStep": ((totals or {}).get("recursion_tree") or {}).get("step_by_step"),
                },
            )
            if selected_case
            else create_section("missing_data"),
        ...
        "iterative": create_section(
            ...
            {
                "lineCostTable": {
                    "worst": ((normalized_cases["worst"] or {}).get("byLine")) if isinstance(normalized_cases["worst"], dict) else None,
                    "best": ((normalized_cases["best"] or {}).get("byLine")) if isinstance(normalized_cases["best"], dict) else None,
                    "avg": ((normalized_cases["avg"] or {}).get("byLine")) if isinstance(normalized_cases["avg"], dict) else None,
                },
                "summations": {
                    "worst": (((normalized_cases["worst"] or {}).get("totals") or {}).get("T_open")) if isinstance(normalized_cases["worst"], dict) else None,
                    "best": (((normalized_cases["best"] or {}).get("totals") or {}).get("T_open")) if isinstance(normalized_cases["best"], dict) else None,
                    "avg": (((normalized_cases["avg"] or {}).get("totals") or {}).get("T_open")) if isinstance(normalized_cases["avg"], dict) else None,
                },
                "simplificationSteps": {
                    "worst": (((normalized_cases["worst"] or {}).get("totals") or {}).get("procedure")) if isinstance(normalized_cases["worst"], dict) else None,
                    "best": (((normalized_cases["best"] or {}).get("totals") or {}).get("procedure")) if isinstance(normalized_cases["best"], dict) else None,
                    "avg": (((normalized_cases["avg"] or {}).get("totals") or {}).get("procedure")) if isinstance(normalized_cases["avg"], dict) else None,
                },
                "asymptoticProcedure": {
                    "worst": (((normalized_cases["worst"] or {}).get("totals") or {}).get("notes")) if isinstance(normalized_cases["worst"], dict) else None,
                    "best": (((normalized_cases["best"] or {}).get("totals") or {}).get("notes")) if isinstance(normalized_cases["best"], dict) else None,
                    "avg": (((normalized_cases["avg"] or {}).get("totals") or {}).get("notes")) if isinstance(normalized_cases["avg"], dict) else None,
                },
                "trace": create_section(
                    "available",
                    {
                        case_name: (
                            {
                                ...
                                "summary": ((trace.get("trace") or {}).get("summary")),
                                "diagnostics": ((trace.get("trace") or {}).get("diagnostics")),
                                "callTreeSource": ((trace.get("trace") or {}).get("callTreeSource"))
                ...
                "loopInvariant": create_section("available", loop_invariant)
                if loop_invariant
                else mark_not_implemented(SNAPSHOT_NOT_IMPLEMENTED_TODOS["loopInvariant"]),
            },
        ),
        "recursive": create_section(
            "available",
            {
                "recurrence": create_section("available", normalized_recurrence),
                "selectedMethod": create_section("available", normalized_recurrence.get("method"))
                if normalized_recurrence and normalized_recurrence.get("method")
                else create_section("missing_data"),
                "methodsAvailable": create_section("available", methods_available)
                if methods_available
                else create_section("missing_data"),
                "methodDetails": create_section("available", method_details)
                if method_details
                else create_section("missing_data"),
                "presentation": recursive_presentation,
                "rootsAndMultiplicities": create_section(
                    "available",
                ...
                "stepByStep": create_section("available", selected_step_by_step)
                if selected_step_by_step
                else create_section("not_supported"),
                "closedForm": create_section(
                    "available",
                    {
                        "homogeneousSolution": ((totals.get("characteristic_equation") or {}).get("homogeneous_solution")),
                        "particularSolution": ((totals.get("characteristic_equation") or {}).get("particular_solution")),
                        "generalSolution": ((totals.get("characteristic_equation") or {}).get("general_solution")),
                        "closedForm": ((totals.get("characteristic_equation") or {}).get("closed_form")),
                        "theta": ((totals.get("characteristic_equation") or {}).get("theta")),
                        "baseCases": ((totals.get("characteristic_equation") or {}).get("base_cases")),
                    },
                )
                if isinstance(totals, dict) and totals.get("characteristic_equation")
                else create_section("not_supported"),
                "recursionTreeSerializable": create_section(
                    "available", totals.get("recursion_tree")
                )
                if isinstance(totals, dict) and totals.get("recursion_tree")
                else create_section(
                    "not_implemented",
        ...
        "comparative": {
            "llm": create_section("available", comparative_llm)
            if comparative_llm
            else create_section("not_requested"),
            "gpuCpu": create_section("available", comparative_gpu_cpu)
        ...
        "institutional": {
            ...
            "generalLimitations": DEFAULT_GENERAL_LIMITATIONS_ES
            if locale == "es"
