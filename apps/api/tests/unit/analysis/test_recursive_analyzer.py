...
class TestRecursiveAnalyzerDPValidation:
    ...
    def test_detect_linear_recurrence_no_linear(self):
        ...
        if result:
            assert not result.get("is_linear", True)
    ...
    def test_apply_characteristic_equation_method_fibonacci(self):
        ...
        assert "equation" in char_eq
        assert "roots" in char_eq
    ...
    def test_detect_applicable_methods_success(self):
        ...
        if result.get("ok"):
            ...
            assert isinstance(result["applicable_methods"], list)
            assert len(result["applicable_methods"]) > 0
        else:
