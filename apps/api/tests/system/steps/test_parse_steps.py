...
@scenario(
    f"{FEATURE_DIR}/{FEATURE}", "Parsear codigo con bloque sin cerrar devuelve error"
)
...
def post_parse_valid(client, response_ctx):
    response_ctx["response"] = client.post(
        "/grammar/parse", json={"source": SOURCES["valid"]}
    )
...
def post_parse_unclosed(client, response_ctx):
    response_ctx["response"] = client.post(
        "/grammar/parse", json={"source": SOURCES["unclosed"]}
    )
