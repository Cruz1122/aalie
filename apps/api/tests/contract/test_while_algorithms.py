...
# Listas filtradas para tests que no aplican a O(1) o sin expectativa.
ALGORITHMS_NOT_O1 = [a for a in ALGORITHMS if a[0] not in ALGORITHMS_O1]
...
ALGORITHMS_WITH_EXPECTED = [a for a in ALGORITHMS if a[0] in EXPECTED_WORST_COMPLEXITY]
...
class TestWhileAlgorithms:
    ...
    def test_worst_case_matches_expected_complexity(self, name: str, source: str):
        ...
        expected = EXPECTED_WORST_COMPLEXITY[name]
        if name in ALGORITHMS_KNOWN_O1_BUG:
            pytest.xfail(
