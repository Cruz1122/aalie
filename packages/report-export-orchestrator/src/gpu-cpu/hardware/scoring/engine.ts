  ...
  (f) =>
    f.hasWhile && f.dataDependentConditions > 0
      ? {
          ...
          evidence: {
            kind: "veto",
            message: "WHILE loop with data-dependent condition inhibits GPU parallelism",
          },
        }
      : null,
  (f) =>
    f.hasEarlyReturn
      ? {
          ...
          evidence: {
            kind: "veto",
            message: "Early return inside a loop (search/pruning) is incompatible with SIMD execution",
          },
        }
      : null,
  (f) =>
  ...
  (f) =>
    f.graphLikeTraversalSignals > 0 || f.pointerOrObjectAccesses > 2
      ? {
          ...
          evidence: {
            kind: "veto",
            message: "Graph traversal or pointer-chasing: irregular memory; not GPU-friendly",
          },
        }
      : null,
  (f) =>
    f.hasRecursion && f.recursiveFanOut <= 1 && f.hasEarlyReturn
      ? {
          ...
          evidence: {
            kind: "veto",
            message: "Single-branch recursion with early exit is not GPU-suitable",
          },
        }
      : null,
];
...
function applyScoreBlocks(
  ...
  const unknownCount = [f.memoryRegularity, f.controlRegularity, f.dependencyStrength].filter(
    (v) => v === "unknown",
  ).length;
