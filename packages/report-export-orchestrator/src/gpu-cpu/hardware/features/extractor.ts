...
/** Check if the assignment is a loop-carried: A[i] = f(A[i-1]) — target has same base as a stencil-read */
...
/** Check if the binary has indirect indexing: A[B[i]] */
...
function getChildren(node: AstNode): AstNode[] {
  switch (node.type) {
    ...
    case "Return":
      return (node as Return).value ? [(node as Return).value as AstNode] : [];
    case "If": {
      const n = node as If;
      const r: AstNode[] = [n.test, ...(n.consequent.body as AstNode[])];
      if (n.alternate) r.push(...((n.alternate.body as AstNode[]) || []));
      return r;
    }
    case "Block":
      return (node as Block).body as AstNode[];
    case "Field":
      return [(node as Field).target as AstNode];
    default:
      return [];
  }
}
...
function visitNode(node: AstNode, ctx: ExtractionCtx): void {
  ...
  switch (node.type) {
    ...
    case "Assign": {
      ...
      if (ctx.insideLoop) {
        if (detectScalarReduction(n)) ctx.scalarReductions++;
      ...
      visitNode(n.value as AstNode, ctx);
      break;
    }
    case "Return": {
      const n = node as Return;
      if (ctx.insideLoop) ctx.hasEarlyReturn = true;
      if (n.value) visitNode(n.value as AstNode, ctx);
      break;
    }
    case "Call": {
      const n = node as Call;
      if (n.callee === ctx.functionName) ctx.recursiveCalls++;
      ...
      for (const arg of n.args) visitNode(arg as AstNode, ctx);
      break;
    }
    case "Field": {
      ctx.pointerAccesses++;
      visitNode((node as Field).target as AstNode, ctx);
      break;
    }
    case "Index": {
      const n = node as Index;
      if (n.index && isIndirectIndex(n.index)) ctx.indirectIndexed++;
      visitNode(n.target as AstNode, ctx);
      if (n.index) visitNode(n.index as AstNode, ctx);
      break;
    }
    default:
      for (const child of getChildren(node)) visitNode(child as AstNode, ctx);
  }
}

function visitBlock(block: Block, ctx: ExtractionCtx): void {
  for (const stmt of block.body) visitNode(stmt as AstNode, ctx);
}

function detectDivideAndConquer(_procDef: ProcDef, _ctx: ExtractionCtx): boolean {
  // Simplified: keep deterministic pipeline compiling.
  return false;
}
...
function classifyControlRegularity(ctx: ExtractionCtx): "regular" | "mixed" | "irregular" | "unknown" {
  ...
  if (ctx.hasEarlyReturn || branchRatio > 0.6) return "irregular";
...
export function extractFeatures(ast: Program): HardwareFeatures {
  ...
  const branchDensity = merged.loopCount > 0 ? merged.branchInLoop / merged.loopCount : 0;
